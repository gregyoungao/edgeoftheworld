...
// AwakeningPresaleShader — WebGL "Waves" background.
// Replaces the previous animated CSS gradient with a live shader while
// preserving the same orange (top) → purple (bottom) palette.
...
vec3 shade(vec2 uv, vec2 p, float t) {
  ...
  float biased = pow(clamp(uv.y, 0.0, 1.0), 0.42);
  float y = biased
...
// Colour recipe rebuilt from the previous CSS gradient so the palette reads
// identically: orange up top fading through cream into deep purple at the
// bottom. palette(y=0) uses colors[0]; palette(y=1) uses the last colour, and
// gl_FragCoord.y (and therefore uv.y) is 0 at the bottom of the canvas and 1
// at the top in WebGL — so index 0 is the bottom (deep purple) and the last
// index is the top (orange).
const UNIFORMS = {
  colors: [
    [0.2392, 0.1843, 0.3608], // #3d2f5c — bottom
    [0.4196, 0.3569, 0.5843], // #6b5b95
    [0.7216, 0.6588, 0.8471], // #b8a8d8
    [1.0000, 0.9608, 0.9020], // #fff5e6
    [0.9569, 0.6431, 0.3765], // #f4a460 — top (repeated to fill 8 slots)
...
export function AwakeningPresaleShader({ className }: { className?: string }) {
